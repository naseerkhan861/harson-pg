...
async function postJson(
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `YiBai 管理端接口 ${pathname} 返回了无法解析的响应，HTTP 状态码：${response.status}`
      );
    }

    console.info(
      `[YiBai 管理端] 请求完成：POST ${pathname}`,
      {
        durationMs:
          Date.now() - startedAt,
    ...
    if (error.name === "AbortError") {
      ...
      throw new Error(
        `YiBai 管理端接口 ${pathname} 超时（${timeoutMs} 毫秒）`
      );
    }

    console.warn(
      `[YiBai 管理端] 请求失败：POST ${pathname}`,
      {
        durationMs:
          Date.now() - startedAt,
